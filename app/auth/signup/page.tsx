...
export default function SignUpPage() {
  ...
  // notifications
  const [notifPrimary] = useState<'alarm'>('alarm'); // fixed primary
  const [notifSecondary, setNotifSecondary] = useState<string[]>([]);

  // pill details
  const [pillTime, setPillTime] = useState<string>('08:00');
  const [pillType, setPillType] = useState<string>('combined');
  const [packStart, setPackStart] = useState<string>('');
  const [activePills, setActivePills] = useState<number>(21);
  const [inactivePills, setInactivePills] = useState<number>(7);
  const [remindPlacebo, setRemindPlacebo] = useState<boolean>(true);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (error || !data.user) {
      setLoading(false);
      return setMsg(error?.message || 'Sign up failed.');
    }

    // 2) store profile answers
    const { error: insertErr } = await supabase.from('profiles').insert({
      user_id: data.user.id,
      full_name: fullName,
      phone,
      notif_primary: notifPrimary,
      notif_secondary: notifSecondary,
      pill_time: pillTime + ':00', // 'HH:MM:SS'
      pill_type: pillType,
